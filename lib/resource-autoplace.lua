...
local resource_autoplace_all_patches = table.deepcopy(data.raw["noise-function"]["resource_autoplace_all_patches"])

local distance_factor_variable_name = "dist_factor"

local distance_add_variable_name = "distance_add"

rro.soft_insert(resource_autoplace_all_patches.parameters,distance_factor_variable_name)
rro.soft_insert(resource_autoplace_all_patches.parameters,distance_add_variable_name)
local function sub_distance(expression)
    return string.gsub(expression,"distance","(distance*".. distance_factor_variable_name .." + ".. distance_add_variable_name ..")")
end

local function sub_distance_function(expression)
    return string.gsub(expression,"distance","(distance,".. distance_factor_variable_name ..")")
end
...
local function resource_autoplace_settings(params)
  ...
  local richness_distance_factor = params.richness_distance_factor or 1 --Added by Muluna 7/14/25
  local size_multiplier = params.size_multiplier or 1
  ...
  {
    {
      ...
      expression = "muluna_resource_autoplace{base_density = " .. params.base_density .. ",\z
                                                   ...
                                                   size_multiplier = " .. size_multiplier .."*var('control:" .. autoplace_control_name .. ":size'),\z
                                                   ...
                                                   starting_rq_factor = " .. ((params.starting_rq_factor_multiplier or 1) / 7) .. ",\z
                                                   distance_add                 = "..                     distance_add                                       .. ",\z
                                                   dist_factor = " .. (params.richness_distance_factor or 1) .. "}"
    }
  }
  ...
  local richness_distance_exp = "max((" .. (local_expressions.double_density_distance -
                                     ...
                                     "+".. richness_distance_factor .. "*distance)/" .. (local_expressions.double_density_distance * 2) .. ",1)"
